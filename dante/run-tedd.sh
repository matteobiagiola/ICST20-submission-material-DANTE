...
function checkMainClassName(){
	...
	if [[ $main_class_name != "validate_deps_original_order" \
	    && $main_class_name != "validate_deps_moea" \
	    && $main_class_name != "validate_deps_sub_use" \
	    && $main_class_name != "validate_deps_coverage" \
	    && $main_class_name != "refine_original_order" \
	    && $main_class_name != "refine_sub_use" \
	    && $main_class_name != "refine_coverage" \
	    && $main_class_name != "refine_moea" ]]; then
...
function setExtractionStrategy() {
    ...
    if [[ $extraction_strategy != "moea" && $extraction_strategy != "original_order"\
        && $extraction_strategy != "coverage" && $extraction_strategy != "sub_use" ]]; then
...
./setup-application.sh $application_name $crawljax_element_strategy
...
if [[ $main_class_name == "validate_deps_original_order" ]]; then
    setExtractionStrategy original_order
    java -Xms2048m -Xmx2048m -cp $classpath \
        com.dante.tedd.validation.ComputeAndValidateDependencies \
            > $HOME/Desktop/logs"_"$main_class_name"_"$application_name.txt \
            2> $HOME/Desktop/errors"_"$main_class_name"_"$application_name.txt
    status=${?}
elif [[ $main_class_name == "validate_deps_moea" ]]; then
    setExtractionStrategy moea
    java -Xms2048m -Xmx2048m -cp $classpath \
        com.dante.tedd.validation.ComputeAndValidateDependencies \
        > $HOME/Desktop/logs"_"$main_class_name"_"$application_name.txt \
            2> $HOME/Desktop/errors"_"$main_class_name"_"$application_name.txt
    status=${?}
elif [[ $main_class_name == "validate_deps_coverage" ]]; then
    setExtractionStrategy coverage
    java -Xms2048m -Xmx2048m -cp $classpath \
        com.dante.tedd.validation.ComputeAndValidateDependencies \
        > $HOME/Desktop/logs"_"$main_class_name"_"$application_name.txt \
            2> $HOME/Desktop/errors"_"$main_class_name"_"$application_name.txt
    ...
    setExtractionStrategy original_order
    java -Xms2048m -Xmx2048m -cp $classpath \
        com.dante.tedd.validation.ValidateDependencies \
        > $HOME/Desktop/logs"_"$main_class_name"_"$application_name.txt \
            2> $HOME/Desktop/errors"_"$main_class_name"_"$application_name.txt
    status=${?}
elif [[ $main_class_name == "refine_coverage" ]]; then
    setExtractionStrategy coverage
    java -Xms2048m -Xmx2048m -cp $classpath \
        com.dante.tedd.validation.ValidateDependencies \
        > $HOME/Desktop/logs"_"$main_class_name"_"$application_name.txt \
    ...
    java -Xms2048m -Xmx2048m -cp $classpath \
        ...
        > $HOME/Desktop/logs"_"$main_class_name"_"$application_name.txt \
            2> $HOME/Desktop/errors"_"$main_class_name"_"$application_name.txt
    status=${?}
elif [[ $main_class_name == "refine_moea" ]]; then
    setExtractionStrategy moea
    java -Xms2048m -Xmx2048m -cp $classpath \
        com.dante.tedd.validation.ValidateDependencies \
        > $HOME/Desktop/logs"_"$main_class_name"_"$application_name.txt \
...
if [[ $status == 50 && $collect_stats == "true" ]]; then
    ...
    cp $dependency_graph_path/$dependency_graph "$dependency_graph_path/graph-initial-$application_name.txt"
fi
...
if [[ $status == 50 && $collect_stats == "true" ]]; then
    ...
    elif [[ $main_class_name == *moea ]]; then
        new_main_class_name=$new_main_class_name"moea"
    fi
    echo New main class name: $new_main_class_name
    run-tedd.sh $new_main_class_name $application_name $start_container $collect_stats $crawljax_element_strategy
fi
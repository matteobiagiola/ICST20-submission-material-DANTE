...
function setApplicationRelatedVariables(){
    ...
    if [[ $application_name_local == "petclinic" ]]; then
		./setup-application.sh petclinic fired
    elif [[ $application_name_local == "phoenix" ]]; then
        ./setup-application.sh phoenix checked
    elif [[ $application_name_local == "splittypie" ]]; then
        ./setup-application.sh splittypie checked
    elif [[ $application_name_local == "dimeshift" ]]; then
        ./setup-application.sh dimeshift fired
    elif [[ $application_name_local == "retroboard" ]]; then
        ./setup-application.sh retroboard fired
    else
        echo Unknown application name: $application_name_local
		exit 1
	fi
...
function runJavaMainClass() {
    ...
    java -Xms2048m -Xmx2048m -cp $classpath \
        com.dante.suitegenerator.atusa.$main_class_name_local \
...
function setStringOfExecutedTests() {
    local log_file=$1
    local byte_string=$(grep -i "Suite execution result as byte string:" $log_file | awk -F": " '{print $2}')
    if [[ $os == "Darwin" ]]; then
        sed -i "" "s/suite_execution_result_byte_string=.*/suite_execution_result_byte_string=$byte_string/g" $properties_file
    else
        sed -i "s/suite_execution_result_byte_string=.*/suite_execution_result_byte_string=$byte_string/g" $properties_file
    fi
}
...
main_class_name=RunAtusaTestsInIsolation
runJavaMainClass $application_name $main_class_name
setStringOfExecutedTests $HOME/Desktop/logs"_"$main_class_name"_"$application_name.txt
main_class_name=MeasureCoverageOfAtusaTests
runJavaMainClass $application_name $main_class_name

...
function setGraphPath() {
    local application_name_local=$1
    local graph_path_local="$HOME/workspace/dante/applications/$application_name_local/testsuite-$application_name_local/src/main/resources/graph-original-order-$application_name_local.txt"
	if [[ $os == "Darwin" ]]; then
        sed -i "" "s%[[:<:]]graph_path[[:>:]]=.*%graph_path=$graph_path_local%g" $properties_file
    else
        sed -i "s%\<graph_path\>=.*%\<graph_path\>=$graph_path_local%g" $properties_file
    fi
}
...
function runJavaMainClass() {
    ...
    java -Xms2048m -Xmx2048m -cp $classpath \
        com.dante.crawling.$main_class_name_local \
...
function setTestSuitePath() {
    ...
    local test_suite_path_local="$HOME/workspace/ICST20-submission-material-DANTE/dante/applications/$application_name_local/testsuite-$application_name_local/src/main/java/tests/GeneratedTestSuite$element_strategy_local.java"
	if [[ $os == "Darwin" ]]; then
...
function setDependencyGraphPath() {
    local application_name_local=$1
    local dependency_graph_path_local="$HOME/workspace/ICST20-submission-material-DANTE/dante/applications/$application_name_local/testsuite-$application_name_local/src/main/resources"
	if [[ $os == "Darwin" ]]; then
...
if test $# -lt 1 ; then echo 'ARGS: application_name (petclinic|splittypie|retroboard|phoenix|dimeshift|ecommerce), plot_pareto_front (true|false), pop_size (num), max_evaluations (num)' ; exit 1 ; fi
...
main_class_name=NSGAIIBinaryRunner
java -cp $classpath com.dante.filtering.biobjective.$main_class_name \

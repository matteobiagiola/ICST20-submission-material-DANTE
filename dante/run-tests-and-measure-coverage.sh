...
function runJavaMainClass() {
    ...
    java -cp $classpath \
        com.dante.comparison.$main_class_name_local \
...
function checkTechnique(){
	...
	if [[ $technique_local != "dante" && $technique_local != "crawljax" && $technique_local != "atusa" ]]; then
...
function setTestSuitePath() {
    ...
    if [[ $technique_local == "atusa" ]]; then
        ...
        test_suite_path_local="$HOME/workspace/ICST20-submission-material-DANTE/dante/applications/$application_name_local/testsuite-$application_name_local/src/main/java/tests/AtusaGeneratedTestSuite$element_strategy_local"K$k_local
    elif [[ $technique_local == "dante" || $technique_local == "crawljax" ]]; then
        test_suite_path_local="$HOME/workspace/ICST20-submission-material-DANTE/dante/applications/$application_name_local/testsuite-$application_name_local/src/main/java/tests/GeneratedTestSuite$element_strategy_local"
    else
...
function setCoverageReportDirectoryPath() {
    ...
    local coverage_reports_directory_path_local="$HOME/workspace/ICST20-submission-material-DANTE/dante/applications/$application_name_local/testsuite-$application_name_local/all-coverage-reports"
	if [[ $os == "Darwin" ]]; then
...
if test $# -lt 3 ; then echo 'ARGS: application_name (petclinic|splittypie|retroboard|phoenix|dimeshift|ecommerce), headless (true|false), technique (dante|crawljax|atusa), extraction_strategy (biobjective|coverage_driven --> only when dante is chosen as technique)' ; exit 1 ; fi

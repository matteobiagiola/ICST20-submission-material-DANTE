...
/**
 * Crawljax runner for the MRBS web application. The crawl will produce
 * output using the {@link CrawlOverview} plugin.
 */
public final class MrbsRunner {
    private final static Logger logger = Logger.getLogger(MrbsRunner.class.getName());

    static final long WAIT_TIME_AFTER_EVENT = 500;
    static final long WAIT_TIME_AFTER_RELOAD = 500;

    public final static String URL = "http://localhost:3000/mrbs/web";

    static final long MAX_RUNTIME = 10;  // minutes
...
//        builder.setStateVertexFactory(new RTEDStateVertexFactory(0.04099));
        ...
        File customOutput = new File("out" + File.separator + "mrbs-acrossapp-pdiff");
